...
namespace Cdmx.Scg.Sancionados.Web.Providers
{
    public class JwtProvider
    {
        ...
        public async Task<string> GetTokenAsync(string username, string password, string clientId, string deviceId)
        {
            using (var client = new HttpClient())
            {
                ...
                {
                    throw new AuthenticationException("El usuario o contraseña son incorrectos");
                }
                else
                {
                    throw new Exception("Error al intentar validar los datos");
        ...
        public ClaimsIdentity CreateIdentity(bool isAuthenticated, string userName, dynamic payload, string acessToken)
        {
            //decode the payload from token
            //in order to create a claim            
            //string userId = payload.nameid; //TODO:Revisar si lo tiene el token al generarlo
            ...
            jwtIdentity.AddClaim(new Claim(ClaimTypes.Name, userName));
            //jwtIdentity.AddClaim(new Claim(ClaimTypes.Sid, payload.nameid));
            jwtIdentity.AddClaim(new Claim("AcessToken", acessToken));
